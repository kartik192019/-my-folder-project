...
class JobDescriptionAnalyzer:
    ...
    def _get_personal_info_prompt(self, text):
        """Generate prompt for personal data extraction from JD"""
        return f"""
        You are an expert information extraction algorithm. Your task is to precisely extract specified details from the provided job description text.

        Extract the following personal requirements and return ONLY a valid JSON object with these exact keys:

        {{
            "preferred_city": "preferred city if mentioned, otherwise null",
            "mandatory_city": "mandatory city if mentioned, otherwise null",
            "preferred_age": "preferred age if mentioned, otherwise null",
            "mandatory_age": "mandatory age if mentioned, otherwise null",
            "preferred_gender": "preferred gender if mentioned, otherwise null",
            "mandatory_gender": "mandatory gender if mentioned, otherwise null"
        }}

        **Extract the following attributes:**
        1. **Location/City:** The primary work location or city.
        2. **Gender:** Explicitly stated gender (e.g., "Male", "Female", or other specified).
        3. **Age:** The specific age in years if mentioned (number of years from date of birth till today). Capture its status (mandatory/preferred).

        **Extraction Rules:**
        * **Mandatory/Preferred Status:** For all extracted parameters, determine if they are **mandatory** or **preferred**. If not explicitly stated, assume the parameter is **mandatory**.
        * **Age vs. Experience:** **Do not confuse required experience in years with age.** Age is explicitly about a person's years alive.
        * **Omission:** If a specific attribute's value is not found in the text, use null.
    ...
    def _get_attributes_prompt(self, text):
        ...
        return f"""
        You are an expert extraction algorithm. Extract job requirements from this job description text.
        ...
        **Extraction Rules:**
        - Provide detailed, comprehensive descriptions for each category
        - Include specific requirements mentioned in the job description
        - If a category is not mentioned, write "Not specified" or "Not available"
        - Use natural language descriptions, not bullet points
        - Be thorough and include all relevant details from the text
    ...
    def analyze_job_description(self, job_description_text):
        ...
        try:
            ...
            personal_prompt = self._get_personal_info_prompt(cleaned_text)
            logger.info("Calling AI for personal information extraction...")
            personal_info = self.call_llm(personal_prompt)
...
def analyze_job_description():
    ...
    try:
        ...
        return jsonify({
            ...
            'jd_id': jd_id,
            'analysis_summary': {
                'personal_info_extracted': len([k for k, v in combined_analysis.get('personal_info', {}).items() if v is not None]),
